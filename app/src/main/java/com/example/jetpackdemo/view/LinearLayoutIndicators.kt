...
import com.example.jetpackdemo.databinding.LayoutIndicatorsBinding

private const val TAG = "LinearLayoutIndicators"
class LinearLayoutIndicators : LinearLayout {
    private  var binding: LayoutIndicatorsBinding? = null
    ...
    fun createCircle(size: Int) {
        ...
        for (i in 0..size) {
            ...
            Log.d(TAG ,"addView i = $i")
            binding?.linearLayout?.addView(view,i, params)
        }
    }

    fun setSelected(position: Int){
        Log.d(TAG ,"inner setSelected position = $position")
